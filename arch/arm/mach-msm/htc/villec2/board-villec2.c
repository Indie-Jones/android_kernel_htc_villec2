...
static void config_flashlight_gpios(void)
{
	static uint32_t flashlight_gpio_table[] = {
		GPIO_CFG(VILLEC2_TORCH_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
		GPIO_CFG(VILLEC2_FLASH_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	};
...
static struct flashlight_platform_data flashlight_data = {
	...
	.torch 			= VILLEC2_TORCH_EN,
	.flash 			= VILLEC2_FLASH_EN,
...
static uint32_t mhl_usb_switch_ouput_table[] = {
	GPIO_CFG(VILLEC2_GPIO_MHL_USB_SWITCH, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
static void villec2_usb_dpdn_switch(int path)
{
	...
	case PATH_MHL:
	{
		...
		gpio_set_value(VILLEC2_GPIO_MHL_USB_SWITCH, (mhl ^ !polarity) ? 1 : 0);
		break;
	}
	}
...
static int64_t villec2_get_usbid_adc(void)
{
	int mean_adc;
	int adc[5];
	int i, ret = 0;
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.get_adc_cb	= villec2_get_usbid_adc,
...
#endif // CONFIG_SUPPORT_DQ_BATTERY
...
static uint32_t gsbi4_gpio_table[] = {
	GPIO_CFG(VILLEC2_CAM_I2C_SDA, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
...
static uint32_t gsbi5_gpio_table[] = {
	GPIO_CFG(VILLEC2_TP_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(VILLEC2_TP_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi5_gpio_table_gpio[] = {
	GPIO_CFG(VILLEC2_TP_I2C_SDA, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VILLEC2_TP_I2C_SCL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi7_gpio_table[] = {
	GPIO_CFG(VILLEC2_GENERAL_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
...
static uint32_t gsbi10_gpio_table[] = {
	GPIO_CFG(VILLEC2_SENSOR_I2C_SDA, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
	GPIO_CFG(VILLEC2_SENSOR_I2C_SCL, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static int villec2_ts_cy8c_set_rst(int on)
{
	...
	struct pm8058_gpio_cfg tp_rst[] = {
		{ /* TW RST Set LOW */
			PM8058_GPIO_PM_TO_SYS(VILLEC2_TP_RST),
			{
...
static int villec2_ts_cy8c_power(int on)
{
	printk(KERN_INFO "%s():\n", __func__);
	if (on)
		villec2_ts_cy8c_set_rst(1);

	return 0;
}

static int villec2_ts_cy8c_reset(void)
{
	printk(KERN_INFO "[TP] HW reset touch.\n");
	villec2_ts_cy8c_set_rst(0);
	msleep(10);
	villec2_ts_cy8c_set_rst(1);
	msleep(200);
...
struct cy8c_i2c_platform_data villec2_ts_cy8c_data[] = {
	{
		.version = 0x0C,
		.timeout = 1,
		.unlock_attr = 1,
		...
		.abs_width_min = 0,
		.abs_width_max = 512,
		.power = villec2_ts_cy8c_power,
		.gpio_irq = VILLEC2_TP_ATT_N_XB,
		/*.filter_level = {40, 80, 942, 982},*/
		.reset = villec2_ts_cy8c_reset,
	},
...
static void villec2_ts_cy8c_set_system_rev(uint8_t rev)
{
	ssize_t i = 0;

	if (rev >= PVT_VERSION)
		for (i = 0; i < sizeof(villec2_ts_cy8c_data)/sizeof(struct cy8c_i2c_platform_data); i++)
			villec2_ts_cy8c_data[i].auto_reset = 1;
}

static struct i2c_board_info msm_i2c_gsbi5_info[] = {
	{
		I2C_BOARD_INFO(CYPRESS_TMA_NAME, 0x67),
		.platform_data = &villec2_ts_cy8c_data,
		.irq = MSM_GPIO_TO_INT(VILLEC2_TP_ATT_N_XB)
	},
};

static ssize_t villec2_virtual_keys_show(struct kobject *kobj,
...
static struct kobj_attribute villec2_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.cy8c-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &villec2_virtual_keys_show,
};

static struct attribute *villec2_properties_attrs[] = {
	&villec2_virtual_keys_attr.attr,
	NULL
};

static struct attribute_group villec2_properties_attr_group = {
	.attrs = villec2_properties_attrs,
};
...
static struct platform_device rpm_regulator_early_device __devinitdata = {
	...
	.id	= 0,
	.dev	= {
		.platform_data = &villec2_rpm_regulator_early_pdata,
	},
};
...
static struct platform_device rpm_regulator_device __devinitdata = {
	...
	.id	= 1,
	.dev	= {
		.platform_data = &villec2_rpm_regulator_pdata,
	},
};
...
static void __init villec2_early_memory(void)
{
...
static uint32_t msm_spi_gpio[] = {
	GPIO_CFG(VILLEC2_SPI_DO,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VILLEC2_SPI_DI,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VILLEC2_SPI_CS,  1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(VILLEC2_SPI_CLK, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static struct tpa2051d3_platform_data tpa2051d3_pdata = {
	.gpio_tpa2051_spk_en = VILLEC2_AUD_HP_EN,
...
static struct isl29028_platform_data isl29028_pdata = {
	.intr = PM8058_GPIO_PM_TO_SYS(VILLEC2_PLS_INT),
...
static struct i2c_board_info i2c_isl29028_devices[] = {
	{
		...
		.irq = PM8058_GPIO_IRQ(PM8058_IRQ_BASE, VILLEC2_PLS_INT),
	},
};
...
static void __init villec2_init(void)
{
	...
	if (system_rev > 2 || speed_bin == 0x1) {
		htc_headset_pmic_data.key_gpio =
			PM8058_GPIO_PM_TO_SYS(VILLEC2_AUD_REMO_PRES);
	...
	if (properties_kobj) {
          rc = sysfs_create_group(properties_kobj, &villec2_properties_attr_group);
	}
...
#endif
	villec2_ts_cy8c_set_system_rev(system_rev);
...
MACHINE_START(VILLEC2, "villec2")	
	...
	.init_very_early = villec2_early_memory,
	.restart = msm_restart,
MACHINE_END
